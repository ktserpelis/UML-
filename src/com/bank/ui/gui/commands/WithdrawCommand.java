...
public class WithdrawCommand<User,Account,Transaction,Bill> implements Command {
    private final BankFacade<User,Account,Transaction,Bill> facade; private final Account account; private final double amount;
    public WithdrawCommand(BankFacade<User,Account,Transaction,Bill> facade, Account account, double amount) {
        this.facade = facade; this.account = account; this.amount = amount;
    }
    @Override public void execute() { facade.withdraw(account, amount); }
}
